...
public final class SentimentAnalyzer {
    ...
    /**
     * Adjust valence if a token is in {@link tn.esprit.springfever.util.Utils#BOOSTER_DICTIONARY} or is a yelling word (all caps).
     *
    ...
    private static float adjustValenceIfCapital(final String precedingToken, final float currentValence,
        ...
        if (tn.esprit.springfever.util.Utils.BOOSTER_DICTIONARY.containsKey(precedingTokenLower)) {
            scalar = tn.esprit.springfever.util.Utils.BOOSTER_DICTIONARY.get(precedingTokenLower);
    ...
    private static List<String> getLeftGrams(final List<String> tokenList, final int minGramLength,
        ...
        for (int end = startPosition; end > 0; end--) {
            ...
            for (int start = windowStart; start >= ((windowEnd < 0) ? 0 : Math.max(0, windowEnd) + 1); start--) {
                currentSuffix = tokenList.get(start) + tn.esprit.springfever.util.Constants.SPACE_SEPARATOR + currentSuffix;
    ...
    private static List<String> getFirstRightGrams(final List<String> tokenList, final int minGramLength,
        ...
        for (int i = minGramLength; i <= maxGramLength; i++) {
            ...
            currentGram.append(tn.esprit.springfever.util.Constants.SPACE_SEPARATOR).append(tokenList.get(endPosition));
    ...
    /**
     * We check if the idioms present in {@link tn.esprit.springfever.util.Utils#SENTIMENT_LADEN_IDIOMS_VALENCE_DICTIONARY} are present in
    ...
    private static float adjustValenceIfLeftGramsHaveIdioms(final float currentValence,
        ...
        for (String leftGramSequence : leftGramSequences) {
            if (tn.esprit.springfever.util.Utils.SENTIMENT_LADEN_IDIOMS_VALENCE_DICTIONARY.containsKey(leftGramSequence)) {
                newValence = tn.esprit.springfever.util.Utils.SENTIMENT_LADEN_IDIOMS_VALENCE_DICTIONARY.get(leftGramSequence);
                break;
            }
        }
        ...
        for (int i = leftGramSequences.size() - 1; i <= 2; i--) {
            if (tn.esprit.springfever.util.Utils.BOOSTER_DICTIONARY.containsKey(leftGramSequences.get(i))) {
     ...
     * in {@link tn.esprit.springfever.util.Utils#SENTIMENT_LADEN_IDIOMS_VALENCE_DICTIONARY} Adjust the current valence if there are
     * any idioms found.
     *
     * @param currentValence      valence to be adjusted
    ...
    private static float adjustValenceIfIdiomsFound(final float currentValence, final int currentItemPosition,
        ...
        for (String rightGramSequence : rightGramSequences) {
            if (tn.esprit.springfever.util.Utils.SENTIMENT_LADEN_IDIOMS_VALENCE_DICTIONARY.containsKey(rightGramSequence)) {
                newValence = tn.esprit.springfever.util.Utils.SENTIMENT_LADEN_IDIOMS_VALENCE_DICTIONARY.get(rightGramSequence);
            }
        }
    ...
    private static List<Float> getTokenWiseSentiment(final TextProperties textProperties) {
        ...
        for (int currentItemPosition = 0; currentItemPosition < wordsAndEmoticons.size(); currentItemPosition++) {
            ...
            if ((currentItemPosition < wordsAndEmoticons.size() - 1
                    ...
                    || tn.esprit.springfever.util.Utils.BOOSTER_DICTIONARY.containsKey(currentItemLower)) {
             ...
             */
            if (tn.esprit.springfever.util.Utils.WORD_VALENCE_DICTIONARY.containsKey(currentItemLower)) {
                currentValence = tn.esprit.springfever.util.Utils.WORD_VALENCE_DICTIONARY.get(currentItemLower);

                if (LOGGER.isDebugEnabled()) {
                    LOGGER.debug("Current currentItem isUpper(): {}", Utils.isUpper(currentItem));
                    LOGGER.debug("Current currentItem isYelling(): {}", tn.esprit.springfever.processor.TextProperties.isYelling());
                }
                 ...
                 */
                if (Utils.isUpper(currentItem) && tn.esprit.springfever.processor.TextProperties.isYelling()) {
                ...
                while (distance < Constants.MAX_GRAM_WINDOW_SIZE) {
                    ...
                    if ((currentItemPosition > distance)
                            && !tn.esprit.springfever.util.Utils.WORD_VALENCE_DICTIONARY.containsKey(wordsAndEmoticons.get(closeTokenIndex)
                            .toLowerCase())) {
                        ...
                        float gramBasedValence = adjustValenceIfCapital(wordsAndEmoticons.get(closeTokenIndex),
                                currentValence, tn.esprit.springfever.processor.TextProperties.isYelling());
    ...
    private static float adjustValenceIfHasAtLeast(final int currentItemPosition,
        ...
        if (currentItemPosition > 1
                && !tn.esprit.springfever.util.Utils.WORD_VALENCE_DICTIONARY.containsKey(wordsAndEmoticons.get(currentItemPosition - 1)
                .toLowerCase())
            ...
            if (!(wordsAndEmoticons.get(currentItemPosition - 2).toLowerCase()
                ...
                && !tn.esprit.springfever.util.Utils.WORD_VALENCE_DICTIONARY.containsKey(wordsAndEmoticons.get(currentItemPosition - 1).toLowerCase())
